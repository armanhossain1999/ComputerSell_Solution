...
namespace ComputerSell.Order
{
    public partial class OrderView : Form
    {
        ...
        public void LoadData()
        {
            ...
            using (SqlConnection con = new SqlConnection(ConnectionHelper.ConnectionString))
            {
                                                                ...
                                                                INNER JOIN Customers c ON o.CustomerId = c.CustomerId", con))
                {
                    ...
                    da.Fill(ds, "Orders");
                    da.SelectCommand.CommandText = @"SELECT OrderId, ComputerName,Price, Quantity
                                                    FROM OrderItems oi
                                                    INNER JOIN Computers p ON oi.ComputerId = p.ComputerId";
        ...
        {
            int orderid = int.Parse((bso.Current as DataRowView).Row["OrderId"].ToString());
            new OrderItemAdd { OrderId = orderid, MainOrderForm=this }.ShowDialog();
        }
